...
type FileUploaderProps = {
  ...
  orientation?: "horizontal" | "vertical";
  /**
   * called AFTER we send the file to backend and get parsed transactions
   */
  onParsedResult?: (data: {
    count: number;
    moneyIn: any[];
  ...
  (
    ...
    const onDrop = useCallback(
      async (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
        ...
        // 1) update local file state like before
        ...
        // 3) CALL BACKEND for the first / newest file
        const fileToSend = files[0]; // we just send the first dropped file
        if (fileToSend) {
          const toastId = toast.loading("Uploading & parsing statement...");
          try {
            const parsed = await uploadStatement(fileToSend);
            toast.success(
              `Parsed ${parsed.count} transactions (in: ${parsed.moneyIn.length}, out: ${parsed.moneyOut.length}, other: ${parsed.other.length})`,
              { id: toastId }
            );
            ...
            console.error(err);
            toast.error(
              err?.message || "Could not parse statement from server",
              { id: toastId }
            );
          }
        }
      ...
      // eslint-disable-next-line react-hooks/exhaustive-deps
      [reSelectAll, value, onParsedResult]
    );
        ...
        >
          ...
          {dropzoneState.isDragActive && (
            <div className="absolute inset-0 flex items-center justify-center bg-primary-foreground/60 backdrop-blur-xs rounded-lg">
              <p className="text-primary font-medium">Drop a file here.</p>
            </div>
          )}
        </div>
        <input
